...
// async function initialSetGender() {
//     const genderJson = await fetchGender();
//     setGender(genderJson);
// }
// const fetchGender = useCallback(() => {
//   const genderRes = fetch("/genders");
//   const genderJson = genderRes.json();
//   console.log(genderJson);
//   // setGender(genderJson);
//   console.log(gender);
//   return genderJson;
...
const gender = {
  options: [
    { title: "Male", name: "Protestant" },
    { title: "Female", name: "Catholic" },
  ...
  const rDesignation = {
    options: [
      { title: "Protestant", name: "Protestant" },
      { title: "Catholic", name: "Catholic" },
      { title: "Other", name: "Other" },
    ],
    getOptionLabel: (option) => option.title,
  };
  ...
  const [selectedRDesignation, setRDesignation] = React.useState(
    rDesignation[0]
  );
      ...
      <Container>
        ...
        <Autocomplete
          ...
          onChange={(event, selectedRDesignation) => {
            console.log(selectedRDesignation);
            setRDesignation(selectedRDesignation);
          }}
          renderInput={(params) => (
            <TextField
              {...params}
